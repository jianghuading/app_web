...
P Chat（AI 角色扮演 · 互动故事）的产品落地页 / App Store 支持页。纯静态单页，无需构建工具。
...
- 语言代码使用 `xx_XX` 下划线格式，目前支持 `en_US`（英文，默认）与 `zh_CN`（简体中文）；页面默认显示英文，访客可随时切换，选择会保存在浏览器本地。
...
3. 把整个 `P-Chat/` 文件夹一起上传到服务器（index.html、css/、image/、i18n/）。
